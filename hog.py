...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1 (Roll a six sided die with -> six_sided())
    total = 0
    we_got_a_one = 0
    while num_rolls > 0:
        stored_roll = dice()
...
def hefty_hogs(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    function = digit_fn(opponent_score%10)(player_score) 
    if opponent_score == 0:
        #player_score += 1
        return 1
    else:
        while opponent_score > 9:
            ...
            function = digit_fn(opponent_score%10)(function)
        end_result = function % 30
        return end_result
    # END PROBLEM 2
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader != None and leader != last_leader:
        msg = "Player " + str(leader) + " takes the lead by " + str(abs(score0 - score1))
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    result = 0
    for i in range(1, 11):
        average_value = make_averaged(roll_dice, total_samples)
        another_max = average_value(i, dice)
        if another_max > max:
            max = another_max
            result = i
    return result
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    #if hefty_hogs(score, opponent_score) == None:
    #    return num_rolls
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    y = score + hefty_hogs(score, opponent_score)
    if hog_pile(y, opponent_score):
        return 0
